...
export class PessoaModal implements OnInit {
  ...
  constructor(private router: Router, private pessoaService: PessoaService, private contatoService: ContatoService ) {
    ...
    debounceTime(900), // espera 400ms após o usuário parar de digitar
...
/*this.pessoas.forEach(p => {
      if (p.id !== undefined) {
        this.contatoService.listarCelularPorPessoa(p.id).subscribe(celular => {
          this.celularesMap[p.id!] = celular;
        });
      }
    });*/
}
...
implementarBuscaPorNome(): void {
  ...
  this.onFechar();
  this.router.navigate(['/principal/menucadastrar/pessoa']);
}
